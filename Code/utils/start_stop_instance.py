...
parser.add_argument('--org', dest='org', action='store', required=False,
...
instance_list = [instance.id for instance in instances ]
# sys.stdout.write(str(instance_list))
sys.stdout.write(json.dumps(instance_list))
for instance in instances:
    if args.instance_state_name == "stopped":
      print(instance.id)
      ec2.instances.filter(InstanceIds=(instance_list)).start()
    elif args.instance_state_name == "running":
      print(instance.id)
